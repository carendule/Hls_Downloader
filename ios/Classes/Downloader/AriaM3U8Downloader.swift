...
extension AriaM3U8Downloader {
    ...
    public func createLocalM3U8File(withTSCount count: Int = -1) {
        ...
        """
        if M3U8_Entity.EXT_X_KEY != nil {
            let keydata = M3U8_Entity.EXT_X_KEY!.hasPrefix("/") ? M3U8_Entity.EXT_X_KEY! : "/\(M3U8_Entity.EXT_X_KEY!)"
            let key = keydata.components(separatedBy: "/").last!
            header.append("\n#EXT-X-KEY:METHOD=\(M3U8_Entity.METHOD!),URI=\"/\(key)\"\(M3U8_Entity.EXT_X_IV == nil ? "" : ",IV=\(M3U8_Entity.EXT_X_IV!)")\n")
            
        }
        for i in 0..<totalCount {
            let tsdata = M3U8_Entity.TSDATA[i].hasPrefix("/") ? M3U8_Entity.TSDATA[i] : "/\(M3U8_Entity.TSDATA[i])"
            let ts = tsdata.components(separatedBy: "/").last!
...
extension AriaM3U8Downloader {
    fileprivate func downloadKey() {
        ...
        queue.addOperation {
            let url = self.M3U8_Entity.EXT_X_KEY.hasPrefix("http") ? self.M3U8_Entity.EXT_X_KEY! : (self.TRUE_Prefix.appendingPathComponent(self.M3U8_Entity.EXT_X_KEY.hasPrefix("/") ? self.M3U8_Entity.EXT_X_KEY : "/\(self.M3U8_Entity.EXT_X_KEY!)")).absoluteString
            AriaBackgroundManager.shared.manager.download(url) { (url, response) -> (destinationURL: URL, options: DownloadRequest.DownloadOptions) in
                ...
                case .failure(_):
                    ...
                    print("\(self.M3U8_URL.absoluteString):\nKEY ->\(url): ->\n下载失败")
                    #endif
                    self.downloadTSFailureExeBlock?("key.key")
    ...
    fileprivate func downloadTS() {
        for ts in M3U8_Entity.TSDATA {
            ...
            if FileManager.default.fileExists(atPath: fv.path) {
               ...
               // NotificationCenter.post(customeNotification: .DownloadTSSuccessNotification, tag: tag, object: ts)
                self.downloadTSSuccessExeBlock?(ts)
                #endif
                continue
            }
            let semaphore = DispatchSemaphore(value: 0)
            queue.addOperation {
                ...
                AriaBackgroundManager.shared.manager.download(url) { (url, response) -> (destinationURL: URL, options: DownloadRequest.DownloadOptions) in
                    ...
                    case .failure(_):
                        ...
                        self.downloadTSFailureExeBlock?(ts)
                    ...
                    if self.downloadStatus != .isStop {
                        let opCount = self.queue.operations.count
                        let dCount = self.M3U8_Entity.TSDATA.count - opCount
                        self.downloadFileProgressExeBlock?(Float(Float(dCount) / Float(self.M3U8_Entity.TSDATA.count)))
                ...
                if self.downloadStatus != .isStop {
                    let opCount = self.queue.operations.count
                    let dCount = self.M3U8_Entity.TSDATA.count - opCount
                    self.downloadM3U8StatusExeBlock?(dCount, self.M3U8_Entity.TSDATA.count)
        ...
        DispatchQueue.global().async {
            ...
            if self.downloadStatus != .isStop {
                self.downloadM3U8StatusExeBlock?(self.M3U8_Entity.TSDATA.count, self.M3U8_Entity.TSDATA.count)
                ...
                if self.queue.operations.count == 0 {
                    ...
                    self.downloadFileProgressExeBlock?(1.0)
            ...
            for m in splitRS {
                ...
                if m.hasPrefix("#EXT-X-KEY:") {
                    ...
                    for i in 0..<split.count{
                        switch i{
                        case 0:
                            entity.METHOD = self.regexGetSub(pattern: "METHOD=(.+)", str: split[i])
                            continue
                        case 1:
                            entity.EXT_X_KEY = self.regexGetSub(pattern: "URI=\"(.+?)\"", str: split[i])
                            continue
                        case 2:
                            entity.EXT_X_IV = self.regexGetSub(pattern: "IV=(.+)", str: split[i])
                            continue
                        default:
                            break
                        }
...
//                    }
                ...
                if m.hasSuffix(".ts") { entity.TSDATA.append(m) }
            }
    ...
    fileprivate func regexGetSub(pattern: String, str: String) -> String {
